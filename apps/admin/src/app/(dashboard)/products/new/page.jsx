...
export default function AddProductPage() {
    const { register, control, handleSubmit, setValue, watch } = useForm({
        defaultValues: {
            ...
            finish: '',
            brochure: '',
            certifications: [], // keeping for safety if schema demands
            fusionUrl: ''
        }
    });
    ...
    return (
        <div className="max-w-[1600px] mx-auto pb-20 px-4 sm:px-6 lg:px-8">
            <header className="mb-8 flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">New Product</h2>
                    <p className="text-gray-500 dark:text-zinc-400 text-sm mt-1">Add a new item to your catalog.</p>
                </div>

            </header>

            <form id="add-product-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-4xl mx-auto">
                ...
                <section className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl p-6 lg:p-8 shadow-sm space-y-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-semibold text-gray-700 dark:text-zinc-300">Slug (Auto-generated)</label>
                        <input
                            {...register("slug")}
                ...
                <div className="flex items-center justify-end gap-3 pt-6 border-t border-gray-200 dark:border-zinc-800 mt-8">
                    <button type="button" onClick={() => router.back()} className="px-6 py-3 rounded-xl border border-gray-300 dark:border-zinc-700 text-gray-700 dark:text-white font-bold hover:bg-gray-50 dark:hover:bg-zinc-800 transition-all text-sm">
                        Cancel
                    </button>
                    <button type="submit" className="px-8 py-3 rounded-xl bg-brand-red text-white font-bold shadow-lg shadow-brand-red/20 hover:bg-red-600 hover:shadow-brand-red/40 hover:-translate-y-0.5 transition-all duration-300 text-sm">
                        Publish Product
                    </button>
                </div>
